...
export default function App() {
  ...
  const handleEdit = (type, item) => {
    ...
    if (type === "posts") {
      ...
      // Keep existing images safely stored in state so they aren't deleted
  ...
  const handleAdminSubmit = async (e) => {
    ...
    if (adminTab === "posts") {
      ...
      // If new inline files were uploaded, replace the old ones completely.
      // If no new files were uploaded, keep the old inline ones.
          ...
          <>
            ...
            <div className="bg-zinc-900 border border-zinc-800 p-4 rounded-xl space-y-4">
               <div>
                 ...
                 {existingMainImage && (
                    <div className="mb-3">
                       <p className="text-xs text-zinc-400 mb-1">Current Image:</p>
                 ...
                 {existingMainImage && <p className="text-[10px] text-zinc-500 mt-1">Select a new file only if you want to replace the current one.</p>}
               </div>
               ...
               <div className="border-t border-zinc-800 pt-4">
                    ...
                    <div className="mb-3 flex gap-2 overflow-x-auto pb-2">
                       {existingInlineImages.map((img, idx) => (
                          <div key={idx} className="relative">
                             <span className="absolute top-0 left-0 bg-amber-500 text-black text-[10px] font-black px-1.5 py-0.5 rounded-br-lg z-10">[image{idx + 1}]</span>
                             <img src={img} alt={`Inline ${idx + 1}`} className="h-16 w-16 rounded-md object-cover border border-zinc-700 shrink-0" />
                          </div>
                       ))}
                    </div>
                 )}

                 <p className="text-xs text-zinc-400 mb-2">Upload multiple files at once. They will become <b>[image1]</b>, <b>[image2]</b>, etc.</p>
                 ...
                 {existingInlineImages.length > 0 && <p className="text-[10px] text-amber-500 mt-1">Warning: Selecting new files will replace all existing inline images.</p>}
               </div>
            </div>
