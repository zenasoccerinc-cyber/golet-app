...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState("ሱቅ"); // Setting shop as default for testing
  ...
  const handleBotOrderSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`ትዕዛዙ አልተላከም። (Failed to send order.)`);
      }
    } catch (error) {
      alert("ስህተት አጋጥሟል። (Error connecting.)");
    } finally {
  ...
  const renderShop = () => {
    ...
    // Core Sub-tab filtering logic requested in Phase 8
    const finalFiltered =
      shopSubCategory === "ሁሉም"
        ? initialFiltered
    ...
    return (
      <div className="pb-24 pt-2">
        ...
        {/* Layer 2 Sub-Tabs Dropdown (Only appears for specific categories) */}
        {(shopCategory === "ወንዶች" ||
          shopCategory === "ሴቶች" ||
          shopCategory === "ልጆች") && (
          <div className="flex overflow-x-auto space-x-2 mb-4 pb-2 scrollbar-hide bg-zinc-950 p-1.5 border border-zinc-900 rounded-xl">
